...
def update_content_types(unpacked_dir: Path, removed_files: list[str]) -> None:
    """Remove Override entries from Content_Types.xml for files that don't exist."""
    ...
    for line in lines:
        if 'PartName="/' in line:
            # Extract the PartName value
            start = line.find('PartName="') + len('PartName="')
            end = line.find('"', start)
            if start > 0 and end > start:
                part = line[start:end]  # e.g. /ppt/slides/slide9.xml
                fpath = unpacked_dir / part.lstrip("/")
                if not fpath.exists():
                    removed_count += 1
    ...
    if removed_count > 0:
        ...
        print(f"  Removed {removed_count} orphaned Content_Types overrides")
...
def clean_unused_files(unpacked_dir: Path) -> list[str]:
    ...
    # Always clean Content_Types — even if no files were removed this run,
    # there may be stale overrides from a previous deletion
    update_content_types(unpacked_dir, all_removed)

    return all_removed


def _report_orphaned_media(unpacked_dir: Path) -> None:
    """Detect media files referenced only by layouts/masters (not slides or notes).

    PowerPoint may flag these as needing repair. This function reports them
    so the user knows to expect a repair prompt — no data loss occurs.
    """
    slide_referenced = set()
    for rels_dir_name in ["slides", "notesSlides"]:
        rels_dir = unpacked_dir / "ppt" / rels_dir_name / "_rels"
        if not rels_dir.exists():
            continue
        for rels_file in rels_dir.glob("*.rels"):
            content = rels_file.read_text(encoding="utf-8")
            for target in re.findall(r'Target="[^"]*?/media/([^"]+)"', content):
                slide_referenced.add(target)

    media_dir = unpacked_dir / "ppt" / "media"
    if not media_dir.exists():
        return

    all_media = {f.name for f in media_dir.iterdir() if f.is_file()}
    orphaned = sorted(all_media - slide_referenced)

    if orphaned:
        print(f"\n⚠️  {len(orphaned)} media file(s) not referenced by any slide (layout/master only).")
        print("   PowerPoint may show a 'repair' prompt when opening — this is cosmetic, no data loss.")
        if len(orphaned) <= 10:
            for f in orphaned:
                print(f"     {f}")
        else:
...
if __name__ == "__main__":
    ...
    # Report orphaned media that are referenced by layouts/masters but not by any slide.
    # These won't cause data loss but will trigger a "repair" prompt in PowerPoint.
    _report_orphaned_media(unpacked_dir)
